...
<div class="container">
    <p class="text-white text-center pt-5 display-6">即將上映電影</p>

    <div class="row ">
        <div v-for="movie in upcomingMovies.movies" :key="movie.id" class="col-6 col-md-3 col-xl-3">
...
<script>
import { apiUpcomingMovie, apiAddToWatchList } from '../api/api';
...
export default {
    setup() {
        ...
        const upcomingMovies = reactive({ movies: [] });
        ...
        const getUpcomingMovie = async() => {
            try{
                const response = await apiUpcomingMovie(currentPage.value);
                console.log(response);
                upcomingMovies.movies = response.data.results;
